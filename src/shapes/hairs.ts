import { Shape } from './types'
import { darken } from '../utils/colors'

export const hairs: Shape[] = [
  (_color) => `
    <g>
      <path fill="#fef6f4" d="M331.94,100.68c-10.14-27.49-29.79-29.85-53.51-21.45c-5.27-6.73-12.55-12.3-28.43-12.92c-15.88,0.63-23.16,6.2-28.43,12.92c-23.72-8.39-43.36-6.03-53.51,21.45c-38.91,19.86-5.83,84.69,33.09,64.83c10.85,12.03,24.36,19.13,48.85,5.73c24.49,13.39,38,6.3,48.85-5.73C337.76,185.37,370.85,120.53,331.94,100.68z"/>
    </g>
  `,
  (color) => `
    <g>
      <path fill="${darken(color, 30)}" d="M202.23,99.58c34.04-8.03,67.49-7.6,100.38,0.87c9.29,2.39,14.88,8.11,13.74,14.1c-7.53,39.66-60.18,79.18-70.09,71.59c-10.72-8.21,2.3-31.48-9.45-33.16c-10.44-1.49-25.88,15.91-37.83,11.87c-2.73-0.92-4.27-2.74-4.18-4.61c0.68-14.78-2.04-35.12-5.58-46.62C187.41,107.69,192.82,101.8,202.23,99.58z"/>
    </g>
  `,
]
